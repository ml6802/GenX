...
function transmission_capacity_decisions!(EP, inputs::Dict, setup::Dict)
    ...
    if NetworkExpansion == 1
        ...
        elseif setup["DC_OPF"] == 1
            ...
            if setup["ptdf"] == 1
                ##### Note: this could be condensed into one consistently named variable, but I kept it separate to replicate monolithic ptdf branch.
                @variable(EP, vZ_BUILD[l in EXPANSION_LINES, i in 1:(inputs["Max_Trans_Cap"][l])] in Parameter(0))
            else
