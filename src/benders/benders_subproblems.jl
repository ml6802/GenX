...
function solve_subproblem(EP::Model,planning_sol::NamedTuple,planning_variables_sub::Vector{String},inputs)
    ...
    elseif haskey(EP, :cPOWER_FLOW_OPF_RETIRE_FORWARD)
        println("POWER FLOW EXPRESSION = ", EP[:cPOWER_FLOW_OPF_RETIRE_FORWARD].data[1,1])
    else
        println("NO FLOW EXPRESSIONS FOUND")
    end
