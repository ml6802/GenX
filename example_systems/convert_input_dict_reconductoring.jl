...
function build_zonal_inputs(inputs::Dict, node_to_zone_map::Dict, num_zones::Int)
    ...
    for l in sort(collect(keys(z2l_map)))
        old_line = z2l_map[l]
        ...
        if old_line in inputs["CAN_RETIRE_LINES"]
            ...
            cand_line_replacement = l2z_map[old_cand_line]
            existing_to_cand_map[l] = cand_line_replacement
        end
    end

    zonal_inputs["EXISTING_LINES"] = EXISTING_LINES
    zonal_inputs["CANDIDATE_LINES"] = CANDIDATE_LINES
    zonal_inputs["RECONDUCTOR_LINES"] = RECONDUCTOR_LINES
    zonal_inputs["CANNOT_RETIRE_LINES"] = CANNOT_RETIRE_LINES
    zonal_inputs["CAN_RETIRE_LINES"] = CAN_RETIRE_LINES
    zonal_inputs["existing_to_cand_map"] = existing_to_cand_map
    zonal_inputs["L_exist"] = length(EXISTING_LINES)
    zonal_inputs["L_cand"] = length(CANDIDATE_LINES)
    zonal_inputs["L"] = length(EXISTING_LINES) + length(CANDIDATE_LINES)

    @assert zonal_inputs["L"] == size(zonal_inputs["pNet_Map"], 1)
