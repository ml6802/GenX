...
function create_portfolio()
    ...
    t_th = SupplyTechnology{ThermalStandard}(;
        ...
        operation_costs=ThermalGenerationCost(
            variable=FuelCurve(LinearCurve(cheap_th_var_cost), 0.0),
            fixed=0.0,
            start_up=0.0,
            shut_down=0.0,
        ),
    ...
    t_th_exp = SupplyTechnology{ThermalStandard}(;
        ...
        power_systems_type="ThermalStandard",
        operation_costs=ThermalGenerationCost(
            variable=FuelCurve(LinearCurve(exp_th_var_cost), 0.0),
            fixed=0.0,
            start_up=0.0,
            shut_down=0.0,
        ),
    ...
    #### Wind ####
    data = CSV.read(joinpath(@__DIR__, "ts_data.csv"), DataFrame)
    ...
    t_wind = SupplyTechnology{RenewableDispatch}(;
        ...
        operation_costs=ThermalGenerationCost(
            variable=FuelCurve(LinearCurve(0.0), 0.0),
            fixed=wind_op_cost,
            start_up=0.0,
            shut_down=0.0,
        ),
    ...
    retro1 = AggregateRetrofitPotential(
        retrofit_id = 1,
        retrofit_fraction = 0.5
    )

    retire1 = AggregateRetirementPotential(
        retirement_potential = 100.0
    )
    ...
    existing = ExistingCapacity(
        existing_technologies = [PSY.get_name(t) for t in thermal[1:3]]
    )
    ...
    t_stor = StorageTechnology{EnergyReservoirStorage}(;
        ...
        operation_costs=StorageCost(
            charge_variable_cost=CostCurve(value_curve=LinearCurve(0.0), vom_cost=LinearCurve(0.0)),
            discharge_variable_cost=CostCurve(value_curve=LinearCurve(0.0), vom_cost=LinearCurve(0.0)),
            fixed=0.0,
        ),
    ...
    line2 = AggregateTransportTechnology{ACBranch}(
        name="test_branch2",
        start_region=z1,
        end_region=z2,
        capacity_limits=(min=0, max=900),
...
case = joinpath(@__DIR__)
setup, OPTIMIZER = wrap_configure_settings(case)

# Create portfolio with base system
portfolio_5bus = create_portfolio()

# Translate portfolio to GenX inputs
inputs = GenX.load_inputs_portfolio(setup, portfolio_5bus, case)

# Generate model
EP = GenX.generate_model(setup, inputs, OPTIMIZER)

# Solve model
EP, solve_time = GenX.solve_model(EP, setup)

# Write capacities back to portfolio
df_cap = prepare_capacity_df(case, inputs, setup, EP)
final_cap = Dict{String, DataFrame}(
    "stage_1" => df_cap)
GenX.set_capacity!(portfolio_5bus, final_cap)
