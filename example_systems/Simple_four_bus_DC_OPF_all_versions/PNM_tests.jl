using PowerNetworkMatrices, SparseArrays
...
function find_matching_index(a::Vector, b::Vector, x, y)
    ...
    return nothing  # or return [] if you want an empty result instead of nothing
end

function calculate_matrices(inputs::Dict, slack_bus::Int; tol = eps())
    A_I = Int[]
    A_J = Int[]
    A_V = Int8[]
    ...
    B_net_cand = inputs["pDC_OPF_coeff"]
    ...
    for (i, line) in enumerate(net_line_list_cand)
        ...
        for j in 1:num_lines
            ...
            push!(all_ind_lines, new_line_name)
            ind_line_map[new_line_name] = length(ind_line_map)
