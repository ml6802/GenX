...
# Run TDR
TDR_params = Dict("MinPeriods" => 4, "MaxPeriods" => 4, "UseExtremePeriods" => 0)
...
for i in myinputs["NEW_CAP"]
    resource = myinputs["RESOURCES"][i]
    vom = GenX.var_om_cost_per_mwh(resource)
    fom = GenX.fixed_om_cost_per_mwyr(resource)
    inv_cost = GenX.inv_cost_per_mwyr(resource)
    parent(resource)[:var_om_cost_per_mwh] = vom * (0.9 + rand() * .2)
    parent(resource)[:fixed_om_cost_per_mwh] = fom * (0.9 + rand() * .2)
    parent(resource)[:inv_cost_per_mwyr] = inv_cost * (0.9 + rand() * .2)
end
...
optimizer = optimizer_with_attributes(Gurobi.Optimizer, "TimeLimit" => 7200, "MIPGap" => 1e-4)
...
# for k in keys(myinputs["Zonal_Capacity_Results"])
#     new_cap_ids = z_inputs["zonal_to_nodal_new_cap"][k]
#     ids = GenX.get_resource_ids_by_name(myinputs, k)
#     if new_cap_ids != ids 
#         println(k)
#     else
#         println(k, "   ", [GenX.resource_name(myinputs["RESOURCES"][i]) for i in new_cap_ids])
#     end
# end
