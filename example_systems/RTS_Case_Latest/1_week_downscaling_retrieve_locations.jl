...
optimizer = optimizer_with_attributes(Gurobi.Optimizer, "TimeLimit" => 1200, "MIPGap" => 1e-4)
...
for i in myinputs["NEW_CAP"]
    ...
    if new_cap_val > 0
        ...
        if !(region in list_of_regions) && (new_cap_val * GenX.cap_size(resource) > 100 )
            push!(list_of_regions, region)
        end
    end
end

list_of_region_names = [i.name for i in list_of_regions]
print(list_of_region_names)