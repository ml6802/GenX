...
idxs = [[35,69], [34,55,65,66], [80,86,87,98]]
sols = [[5, 1], [2,1,1,3], [1, 1, 1, 1]]
...
for i in 1:num_zones
    ...
    for j in 1:length(idxs[i])
        #println(sols[i][j] * GenX.cap_size(n_input["RESOURCES"][idxs[i][j]]))
        println(GenX.resource_name(n_input["RESOURCES"][idxs[i][j]]))
        new_cap_results[n2g_map[idxs[i][j]]] = sols[i][j] * GenX.cap_size(n_input["RESOURCES"][idxs[i][j]])
    end
end
new_gen_cap_df[!, "NEW_CAP"] = new_cap_results

CSV.write((@__DIR__)*"/new_cap_downscaling_resultsb.csv", new_gen_cap_df)
