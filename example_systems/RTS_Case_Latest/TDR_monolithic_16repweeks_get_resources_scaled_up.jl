...
# Write to CSV
CSV.write((@__DIR__)*"/new_cap_resources_della.csv", df_resources)

# Display the dataframe
println(df_resources)
...
for i in myinputs_copy["NEW_CAP"]
    resource = myinputs_copy["RESOURCES"][i]
    push!(resource_ids, i)
    push!(zone_ids, GenX.zone_id(resource))
    push!(resource_names, GenX.resource_name(resource))
    push!(regions, GenX.region(resource).name)
...
# Create DataFrame
df_resources_scaled_up = DataFrame(
    ResourceID = resource_ids,
    ZoneID = zone_ids,
    ResourceName = resource_names,
    Region = regions
...
# Write to CSV
CSV.write((@__DIR__)*"/new_cap_resources_scaled_up_della.csv", df_resources_scaled_up)
