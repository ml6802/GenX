...
mysetup["settings_path"] = settings_path;
 ...
 myinputs["pTrans_Max"] .*= 1.5
# myinputs["pTrans_Max"][[5,23,24,70,75]] .*= 1/2.5
#myinputs["pD"] .*= 1
# myinputs["Voll"] .*= 10
...
myinputs["Line_Reinforcement_Cap_Size"] = [i*2 for i in myinputs["pTrans_Max"]]
...
optimizer = optimizer_with_attributes(Gurobi.Optimizer, "TimeLimit" => 360, "MIPGap" => 5e-3)
...
if UB_hist1[end] < objective_value(m1)
    ...
    for k in keys(l2l_map)
        old_line = k
        new_line = l2l_map[k]
        val = planning_sol1.values["vNEW_TRANS_CAP_DECISION_INT[$new_line]"]
        fix(m1[:vNEW_TRANS_CAP_DECISION_INT][new_line, 1], val, force = true)
        # end
    end
    optimize!(m1)
end
...
for k in keys(l2z_map)
    ...
    if value(mz[:vNEW_TRANS_CAP_DECISION_INT][new_line, 1]) >0
...
for i in 1:num_zones
    ...
    for k in keys(l2l_map)
        old_line = k
        new_line = l2l_map[k]
        if value(models[i][:vNEW_TRANS_CAP_DECISION_INT][new_line, 1]) >0
            add_edge_data!(dg, fadjlist[k][1], fadjlist[k][2], "red", "new_build")
            add_edge_data!(dg, fadjlist[k][1], fadjlist[k][2], 5, "linewidth")
        end
    end
...
plot_graph(dg, nodecolor = get_node_data(dg, "color"), nodesize = 6, xdim = 500, ydim = 500, linewidth = get_edge_data(dg, "linewidth"), linecolor = get_edge_data(dg, "new_build"), save_fig = false, fig_name = (@__DIR__)*"/zonal_nodal_builds_RTS_repweek_benders.png")
