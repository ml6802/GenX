...
if haskey(mysetup, "IntegerInvestments")
    if mysetup["IntegerInvestments"] == 1
        for i in myinputs["NEW_CAP"]
            ...
            if occursin("CC", GenX.resource_name(resource))
                parent(resource)[:cap_size] = 700
            elseif occursin("PV", GenX.resource_name(resource))
                parent(resource)[:cap_size] = 100
            elseif occursin("NUCLEAR", GenX.resource_name(resource))
                parent(resource)[:cap_size] = 300
            else
...
level_set_bool = parse(Int, ARGS[1])
bilinear_bool = parse(Int, ARGS[2])
...
mysetup["BD_post_warmstart_integer_routine_maxiter"] = 900
mysetup["BD_warmstart_bigM_maxiter"] = 700
