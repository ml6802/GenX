...
line_builds = [1,2,6,18,20,25,27,30,46,47,48,54,55,63,65,66,75,87,111,113]
cap_builds = [9,22,24,60,91,94]
new_cap = [1569.655,379.47244,1569.655,379.4724,379.768,379.768]

lines_PSIP = collect(get_technologies(TransmissionTechnology, p))
adj_list = myinputs["adj_list"][1]
index_to_region = z_inputs["index_to_region"]

df_lines = DataFrame()
psip_start_nodes = []
psip_end_nodes = []
psip_name = []
for i in line_builds
    line = lines_PSIP[i]
    psip_start_node_val = line.start_node.id
    psip_end_node_val = line.end_node.id
    psip_name_val = line.name
    push!(psip_start_nodes, psip_start_node_val)
    push!(psip_end_nodes, psip_end_node_val)
    push!(psip_name, psip_name_val)
end

df_lines[!, "original_line_name"] = psip_name
df_lines[!, "original_start_node_ID"] = psip_start_nodes
df_lines[!, "original_end_node_ID"] = psip_end_nodes
#CSV.write((@__DIR__)*"/new_line_builds.csv", df_lines)

sts_PSIP = union(collect(get_technologies(SupplyTechnology, p)), collect(get_technologies(StorageTechnology, p)))

function find_tech_by_id(supplies, id)
    for i in supplies
        if id == get_id(i)
            return i
        end
...
for (idx, i) in enumerate(cap_builds)
    tech_id = z_inputs["index_to_technology"][i]
    tech = find_tech_by_id(sts_PSIP, tech_id)
    push!(cap_name, tech.name)
end
df_cap[!, "PSIP_generator_name"] = cap_name
df_cap[!, "new_capacity_value"] = new_cap

#CSV.write((@__DIR__)*"/new_capacity_builds.csv", df_cap)
