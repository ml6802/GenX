...
src_path = "../../../src/"
inpath = pwd()
multirun_path = joinpath(inpath, "mga_multirun.jl")
...
if mysetup["ModelingToGenerateAlternatives"] == 1
    if mysetup["ModelingToGenerateAlternativesParallel"] > 0
        value = string(mysetup["ModelingToGenerateAlternativesParallel"])
        val2 = string(mysetup["ModelingToGenerateAlternativesParallel"])
        println("Starting "*value*" parallel MGA runs, each with "*val2*" MGA iterations")
        include(multirun_path)
    else
